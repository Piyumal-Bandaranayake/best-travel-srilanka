        ...
        <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
                {safariPackages.map((safari) => (
                    <div key={safari.id} className="h-full">
                        <ActivityCard 
                            activity={safari} 
                            noLink={true}
                            buttonText="View Details"
                            onBook={() => setSelectedSafari(safari)}
                            onClick={() => setSelectedSafari(safari)}
                            subtitle={safari.location}
                        ...
                        >
                            ...
                            <div className="relative w-full md:w-1/2 h-64 md:h-auto">
                                ...
                                <div className="absolute bottom-4 left-4 text-white md:hidden">
                                     <h3 className="text-2xl font-bold">{selectedSafari.title}</h3>
                                     <div className="flex items-center gap-1 text-sm text-white/80">
                                        <MapPin className="w-4 h-4" />
                                        {selectedSafari.location}
                                     </div>
                                </div>
                            </div>
                            ...
                            <div className="p-8 md:w-1/2 overflow-y-auto">
                                <div className="hidden md:block mb-6">
                                    <h3 className="text-3xl font-black text-gray-900 mb-2">{selectedSafari.title}</h3>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-8">
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                        ...
                                        <p className="font-semibold text-gray-900">{selectedSafari.duration}</p>
                                    </div>
                                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                                        ...
                                        <p className="font-semibold text-gray-900">{selectedSafari.price}</p>
                                    </div>
                                </div>

                                <h4 className="text-lg font-bold text-gray-900 mb-3">About this Safari</h4>
                                <p className="text-gray-600 leading-relaxed mb-8">
                                    {selectedSafari.description}
                                </p>

                                <button 
                                    onClick={() => handleBook(selectedSafari)}
